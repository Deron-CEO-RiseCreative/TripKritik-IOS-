...
- (void)queryTopBusinessInfoForTerm:(NSString *)term category:(NSString *)category radius:(double)radius location:(NSString *)location completionHandler:(void (^)(NSDictionary *topBusinessJSON, NSError *error))completionHandler {

  NSLog(@"Querying the Search API with term \'%@\' and location \'%@'", term, location);

  //Make a first request to get the search results with the passed term and location
    NSURLRequest *searchRequest = [self _searchRequestWithTerm:term category:category radius:radius location:location];
...
- (void)queryBusinessesInfoForTerm:(NSString *)term category:(NSString *)category radius:(double)radius location:(NSString *)location completionHandler:(void (^)(NSDictionary *jsonResponse, NSError *error))completionHandler {
 ...
 */
- (NSURLRequest *)_searchRequestWithTerm:(NSString *)term category:(NSString *)category radius:(double)radius location:(NSString *)location {
  NSDictionary *params = @{
//                           @"term": term,
                           @"location": location,
                           @"category_filter": category,
                           @"radius_filter": [NSString stringWithFormat:@"%f", radius]
                           };
